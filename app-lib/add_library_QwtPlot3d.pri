...
defineTest(add_defines_QwtPlot3d){
    ...
    #--------------------------------------------
    #Multi-link 提供 QwtPlot3d 的自有控制宏，
    #留意 QwtPlot3d 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 QwtPlot3d 使用的控制宏，修改 QwtPlot3d 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #Qwt3D比较特殊，使用QWT3D_DLL约束动态编译和链接。这里不定义QWT3D_MAKEDLL代表导入。
    #QwtPlot3d 动态编译时
    contains(DEFINES, QWTPLOT3D_LIBRARY){
        message($${TARGET} build QwtPlot3d dynamic library)
        DEFINES += QT_DLL QWT3D_DLL QWT3D_MAKEDLL
    }
    #QwtPlot3d 静态编译、链接时
    else:contains(DEFINES, QWTPLOT3D_STATIC_LIBRARY){
        message($${TARGET} build-link QwtPlot3d static library)
    }
    #QwtPlot3d 动态链接时
    else:!contains(DEFINES, QWTPLOT3D_LIBRARY){
        message($${TARGET} link QwtPlot3d dynamic library)
