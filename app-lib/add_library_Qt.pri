...
defineTest(add_defines_Qt){
    ...
    #--------------------------------------------
    #Multi-link 提供 Qt 的自有控制宏，
    #留意 Qt 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 Qt 使用的控制宏，修改 Qt 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #Qt 动态编译时
    contains(DEFINES, QT_LIBRARY){
        message($${TARGET} build Qt dynamic library)
    }
    #Qt 静态编译、链接时
    else:contains(DEFINES, QT_STATIC_LIBRARY){
        message($${TARGET} build-link Qt static library)
    }
    #Qt 动态链接时
    else:!contains(DEFINES, QT_LIBRARY){
        message($${TARGET} link Qt dynamic library)
    }
