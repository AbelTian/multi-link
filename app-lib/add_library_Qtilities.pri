...
defineTest(add_defines_Qtilities){
    ...
    #--------------------------------------------
    #Multi-link 提供 Qtilities 的自有控制宏，
    #留意 Qtilities 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 Qtilities 使用的控制宏，修改 Qtilities 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #Qtilities 动态编译时
    contains(DEFINES, QTILITIES_LIBRARY){
        message($${TARGET} build Qtilities dynamic library)
    }
    #Qtilities 静态编译、链接时
    else:contains(DEFINES, QTILITIES_STATIC_LIBRARY){
        message($${TARGET} build-link Qtilities static library)
    }
    #Qtilities 动态链接时
    else:!contains(DEFINES, QTILITIES_LIBRARY){
        message($${TARGET} link Qtilities dynamic library)
    }
