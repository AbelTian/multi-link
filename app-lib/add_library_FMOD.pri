...
defineTest(add_defines_FMOD){
    ...
    #--------------------------------------------
    #Multi-link 提供 FMOD 的自有控制宏，
    #留意 FMOD 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 FMOD 使用的控制宏，修改 FMOD 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #FMOD 动态编译时
    contains(DEFINES, FMOD_LIBRARY){
        message($${TARGET} build FMOD dynamic library)
    }
    #FMOD 静态编译、链接时
    else:contains(DEFINES, FMOD_STATIC_LIBRARY){
        message($${TARGET} build-link FMOD static library)
    }
    #FMOD 动态链接时
    else:!contains(DEFINES, FMOD_LIBRARY){
        message($${TARGET} link FMOD dynamic library)
    }
