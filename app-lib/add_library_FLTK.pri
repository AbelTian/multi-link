...
defineTest(add_include_FLTK){
    ...
    header_path=$$get_add_include(FLTK, FLTK)
...
defineTest(add_defines_FLTK){
    ...
    #--------------------------------------------
    #Multi-link 提供 FLTK 的自有控制宏，
    #留意 FLTK 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 FLTK 使用的控制宏，修改 FLTK 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #FLTK 动态编译时
    contains(DEFINES, FLTK_LIBRARY){
        message($${TARGET} build FLTK dynamic library)
    }
    #FLTK 静态编译、链接时
    else:contains(DEFINES, FLTK_STATIC_LIBRARY){
        message($${TARGET} build-link FLTK static library)
    }
    #FLTK 动态链接时
    else:!contains(DEFINES, FLTK_LIBRARY){
        message($${TARGET} link FLTK dynamic library)
    }
