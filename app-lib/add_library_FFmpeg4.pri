...
defineTest(add_defines_FFmpeg4){
    ...
    #--------------------------------------------
    #Multi-link 提供 FFmpeg4 的自有控制宏，
    #留意 FFmpeg4 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 FFmpeg4 使用的控制宏，修改 FFmpeg4 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #FFmpeg4 动态编译时
    contains(DEFINES, FFMPEG4_LIBRARY){
        message($${TARGET} build FFmpeg4 dynamic library)
    }
    #FFmpeg4 静态编译、链接时
    else:contains(DEFINES, FFMPEG4_STATIC_LIBRARY){
        message($${TARGET} build-link FFmpeg4 static library)
    }
    #FFmpeg4 动态链接时
    else:!contains(DEFINES, FFMPEG4_LIBRARY){
        message($${TARGET} link FFmpeg4 dynamic library)
    }
