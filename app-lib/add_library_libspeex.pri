...
defineTest(add_defines_libspeex){
    ...
    #--------------------------------------------
    #Multi-link 提供 libspeex 的自有控制宏，
    #留意 libspeex 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 libspeex 使用的控制宏，修改 libspeex 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #libspeex 动态编译时
    contains(DEFINES, LIBSPEEX_LIBRARY){
        message($${TARGET} build libspeex dynamic library)
    }
    #libspeex 静态编译、链接时
    else:contains(DEFINES, LIBSPEEX_STATIC_LIBRARY){
        message($${TARGET} build-link libspeex static library)
    }
    #libspeex 动态链接时
    else:!contains(DEFINES, LIBSPEEX_LIBRARY){
        message($${TARGET} link libspeex dynamic library)
    }
