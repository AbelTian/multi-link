...
defineTest(add_defines_freetype2){
    ...
    #--------------------------------------------
    #Multi-link 提供 freetype2 的自有控制宏，
    #留意 freetype2 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 freetype2 使用的控制宏，修改 freetype2 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #freetype2 动态编译时
    contains(DEFINES, FREETYPE2_LIBRARY){
        message($${TARGET} build freetype2 dynamic library)
    }
    #freetype2 静态编译、链接时
    else:contains(DEFINES, FREETYPE2_STATIC_LIBRARY){
        message($${TARGET} build-link freetype2 static library)
    }
    #freetype2 动态链接时
    else:!contains(DEFINES, FREETYPE2_LIBRARY){
        message($${TARGET} link freetype2 dynamic library)
    }
