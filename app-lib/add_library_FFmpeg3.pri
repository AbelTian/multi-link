...
defineTest(add_defines_FFmpeg3){
    ...
    #--------------------------------------------
    #Multi-link 提供 FFmpeg3 的自有控制宏，
    #留意 FFmpeg3 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 FFmpeg3 使用的控制宏，修改 FFmpeg3 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #FFmpeg3 动态编译时
    contains(DEFINES, FFMPEG3_LIBRARY){
        message($${TARGET} build FFmpeg3 dynamic library)
    }
    #FFmpeg3 静态编译、链接时
    else:contains(DEFINES, FFMPEG3_STATIC_LIBRARY){
        message($${TARGET} build-link FFmpeg3 static library)
    }
    #FFmpeg3 动态链接时
    else:!contains(DEFINES, FFMPEG3_LIBRARY){
        message($${TARGET} link FFmpeg3 dynamic library)
    }
