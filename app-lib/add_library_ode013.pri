...
defineTest(add_defines_ode013){
    ...
    #--------------------------------------------
    #Multi-link 提供 ode013 的自有控制宏，
    #留意 ode013 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 ode013 使用的控制宏，修改 ode013 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #ode013 动态编译时
    contains(DEFINES, ODE013_LIBRARY){
        message($${TARGET} build ode013 dynamic library)
    }
    #ode013 静态编译、链接时
    else:contains(DEFINES, ODE013_STATIC_LIBRARY){
        message($${TARGET} build-link ode013 static library)
    }
    #ode013 动态链接时
    else:!contains(DEFINES, ODE013_LIBRARY){
        message($${TARGET} link ode013 dynamic library)
    }
