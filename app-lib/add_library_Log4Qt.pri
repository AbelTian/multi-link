...
defineTest(add_include_Log4Qt){
    ...
    #isEmpty(1):header_path=$$get_add_include(Log4Qt, Log4Qt)
    ...
    #这里添加$${path}下的子文件夹
    #...
    header_path=$$get_add_include(Log4Qt, log4qt)
    command += $${header_path}
...
defineTest(add_defines_Log4Qt){
    ...
    #--------------------------------------------
    #Multi-link 提供 Log4Qt 的自有控制宏，
    #留意 Log4Qt 使用的控制宏
    #--------------------------------------------

    #--------------------------------------------
    #根据 Log4Qt 使用的控制宏，修改 Log4Qt 编译时、链接时的不同的宏配置。编译时，修改前两个判断分支；链接时，修改后两个判断分支。
    #可以用于转换使用不同宏、两套宏控制的链接库。
    #--------------------------------------------
    #Log4Qt 动态编译时
    contains(DEFINES, LOG4QT_LIBRARY){
        message($${TARGET} build Log4Qt dynamic library)
    }
    #Log4Qt 静态编译、链接时
    ...
    #Log4Qt 动态链接时
        ...
        message($${TARGET} link Log4Qt dynamic library)
    }
